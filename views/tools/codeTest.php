...
$this->title = Yii::t('app', 'Code Test');
?>
<style>
.container{ width: 100% }
.well{ padding: 5px 5px }
</style>

<div id="form" class="well" style="min-width:700px; max-width:1200px">
    <?php //$this->widget('myPanelbar', array('selector'=>'cmd', 'title'=>'', 'content'=>'', ));  ?>
    <div class="badge badge-inverse pull-left" onClick="$('#cmd').slideToggle();">Code Test</div>
    <div class="btn-group-xs pull-right" style="padding:0px;">
        <div class="glyphicon glyphicon-expand" title="Maximize" onClick="$('#cmd').attr({rows:'100%'}).parent().animate({height:'100%'}, 'slow');"></div>
        <div class="glyphicon glyphicon-collapse-down" title="Miximize" onClick="$('#cmd').attr({'rows': 20, 'overflow':'auto'}).parent().css({height:'100%'});"></div>
        <div class="glyphicon glyphicon-collapse-up" title="close" onClick=" $('#cmd').slideToggle().parent().css({'height':'auto'});"></div>
    </div>
    <div class="clearfix"></div> 

    <?php echo Html::beginForm(); ?>
    <?php echo Html::textarea("sourcecode", $code, ["id"=>"cmd", 'placeholder'=>"type code..", "rows"=>"20", "style"=>"width:100%;", "class"=>"cmd_line"] ); ?>
    <div class="input-group" style="margin-top:5px; width:200px">
        <?= Html::dropDownList('mode', 'file', ['file'=>'render file', 'eval'=>'eval()'], ['id'=>'mode', 'class'=>"form-control"] ); ?>
        <div class="input-group-btn">
        <?= Html::button('Send', ['class'=>'btn btn-primary', 'onclick'=>'send();']); ?>
        </div>
    </div>
...
$this->registerJs("
    function send(){    
        ...
        $.ajax({
            //url: '../data.php',
            type: 'post',
            data : { cmd: $('#cmd').val(), mode: $('#mode').val() },
            dataType: 'html',
            beforeSend : function() {  
...
<div class="pull-left">Result :</div> <div id="load" class="pull-left loading box-load" style="width:50px;display:none">&nbsp;&nbsp;</div>
<div class="clearfix"></div>
